...
Matching priority (highest to lowest specificity):
  1. canonical_LEDD_med + release  (e.g. carbidopa/levodopa + er)
  2. canonical_LEDD_med + route    (e.g. levodopa + inh)
  3. canonical_LEDD_med alone      (e.g. rasagiline)

Within each tier, longer canonical_LEDD_med strings are tried first so that
...
def _build_ledd_lookup(ledd_df: pd.DataFrame) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    Split the LEDD table into three pre-sorted lookup lists:
      tier1: rows that have canonical_LEDD_med AND release
      tier2: rows that have canonical_LEDD_med AND route
      tier3: rows that have canonical_LEDD_med only (no release, no route)

    Each list is sorted descending by len(canonical_LEDD_med) so longer/more
    specific drug names are matched first.
    """
    tier1, tier2, tier3 = [], [], []

    for _, row in ledd_df.iterrows():
        name    = _clean(row.get("canonical_LEDD_med"))
...
def apply_ledd_mapping(
    df: pd.DataFrame,
    ledd_csv_path: str,
    name_col: str    = "canonical_LEDD_med",
    ...
    ledd_df["canonical_LEDD_med"] = ledd_df["canonical_LEDD_med"].str.lower().str.strip()
